...
main() {
  ...
  head->next = NULL;
  head->prev = NULL;
  // printf("\nEnter the linked list elements(%d to exit and enter atleat one
  // element): ",INT_MAX);

  printf("\nEnter the linked list elements(-9999 to exit and enter atleat one "
         "element): ");
  ...
  if (pos >= 1)
    ...
    printf("Given number is not in the linked list\n");
  pos = linear_search_doubly_linked_list_e(
      temp, to_search); // temp is the last element too
  if (pos >= 1)
